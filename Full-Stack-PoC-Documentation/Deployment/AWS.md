...
> stay in **`k8s/` manifests**, _not_ Terraform. Terraform provisions the
...
It bills per hour the cluster exists, idle or not. The only lever is _time_: create
...
> target groups, and security groups created _by the AWS Load Balancer Controller_
